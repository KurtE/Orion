...
// Actual pins on Orion shield - Note: May special case two other pins if necessary.
#ifdef QUADMODE
...
//--------------------------------------------------------------------
//[OutputExtraServoInfo] Output an extra servo to the packet information
//--------------------------------------------------------------------
#ifdef cExtraServos
short g_sbms1Angle1;
short g_sbms2Angle1;
void ServoDriver::OutputExtraServoInfo(byte ServoIndex, short sServoAngle1)
{
    byte bPin = pgm_read_byte(&caExtraPins[ServoIndex]);
#ifdef APOD_CODE
    if (bPin == (byte)-1)
      g_sbms1Angle1 = sServoAngle1;
    else if (bPin == (byte)-2)
      g_sbms2Angle1 = sServoAngle1;
    else  
#endif
      Orion.setAngle(bPin, sServoAngle1);
}
#endif
...
void ServoDriver::CommitServoDriver(word wMoveTime)
{
  if (g_fDebugServos) {
    ...
    Orion.execute();
#ifdef APOD_CODE
    g_bms1.setAngle(g_sbms1Angle1, wMoveTime);
    g_bms2.setAngle(g_sbms2Angle1, wMoveTime);
#endif
  }
}

//--------------------------------------------------------------------
//[Background Frees all the servos
//--------------------------------------------------------------------
#ifdef OPT_BACKGROUND_PROCESS
void ServoDriver::BackgroundProcess()
{
  // Only needed if Servos hooked up on Arduino pins
#ifdef APOD_CODE
  g_bms1.update();
  g_bms2.update();
#endif
...
void ServoDriver::FreeServos(void)
{
  ...
  for(int ix=0; ix< cExtraServos; ix++) {
    byte bPin = pgm_read_byte(&caExtraPins[ix]);
#ifdef APOD_CODE
    if (bPin == (byte)-1)
      g_bms1.stopPulse();
    else if (bPin == (byte)-2)
...
#ifdef APOD_CODE
    if (bPin == (byte)-1)
      g_bms1.setAngle(g_bms1.queryFBAngle(), 0);
    else if (bPin == (byte)-2)
      g_bms2.setAngle(g_bms2.queryFBAngle(), 0);
    else  
#endif
      Orion.setAngle(bPin, Orion.queryFBAngle(bPin));
  }
#endif
