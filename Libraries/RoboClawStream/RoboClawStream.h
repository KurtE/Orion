#ifndef RoboClawStream_h
#define RoboClawStream_h
...
class RoboClawStream : public Stream
{
...
public:
	// public methods
	RoboClawStream(Stream *pstream, uint32_t tout=10, bool doack=false);	//ack option only available on 3.1.8 and newer firmware
    ...
    using Print::write; // pull in write functionsfrom Print
	...
	bool ReadMinMaxMainVoltages(uint8_t address,uint16_t &minx,uint16_t &maxx);
	bool ReadMinMaxLogicVoltages(uint8_t address,uint16_t &minx,uint16_t &maxx);
...
private:
    ...
    // Added by Kurt
    Stream *_pstream;    // the Real output stream.
    uint32_t _timeout;   // Timeout value
	int timedRead();
