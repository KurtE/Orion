...
public:
	// Lets change to do two part init, as to not do stuff before user program can initialize.
	BMPS2(uint8_t pin, unsigned int updaterate=30, uint8_t pressures=PSABC_ALL) : BMSerial(pin, pin)
	{
        // Move the begin until the first time we do a read, as to not call until setup is in process. 
		lasttime = 0;
	...
	boolean read_ps2(boolean fWait=false);
    void    explicitReadsOnly(boolean fExplictOnly=1)
    {
        explicitOnly = fExplictOnly;
    };    
